%s
% # Лабораторная работа №8: Методы последовательной безусловной минимизации:
% Изучение численных методов нелинейного программирования, 
% относящихся к методам последовательной безусловной минимизации:
% 1. метод внутренних штрафных (барьерных) функций
...
%c 
% штрафные функции ограничения введены!
...
% main ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
fprintf('Лабораторная работа №8: Методы последовательной безусловной минимизации\n'); %с
fprintf('Изучение численных методов нелинейного программирования:\n'); %с
fprintf('1. метод внутренних штрафных (барьерных) функций\n'); %с
fprintf('2. метод внешних штрафных функций\n'); %с
...
fprintf('--------- Метод внутренних штрафных функций: ------------\n\n'); %c
...
switch choose_function
    case 1
        figure('Position', [3*window_offset 3*window_offset full_window_size(3)-window_offset_size ...
            ...
            'Метод внутренних штрафных функций (квадратичная ф.)', 'NumberTitle', 'off'); %c
    case 2
        figure('Position', [7*window_offset 7*window_offset full_window_size(3)-window_offset_size ...
            full_window_size(4)-window_offset_size], 'Name', ...
            strcat('Метод внутренних штрафных функций (ф. Розенброка, alpha = ', num2str(alpha), ')'), ...
            'NumberTitle', 'off'); %c
    case 3
        figure('Position', [3*window_offset 3*window_offset full_window_size(3)-window_offset_size ...
            full_window_size(4)-window_offset_size], 'Name', ...
            'Метод внутренних штрафных функций (ф. Химмельблау)', 'NumberTitle', 'off'); %c
    otherwise
        disp('ERROR!')
end
%xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
...
method_num = 2;
kappa_max = kappa_adjustment(kappa_max, alpha, case_num, method_num);
fprintf('- Метод внешних штрафных функций: -\n\n'); %c
...
switch choose_function
    case 1
        figure('Position', [2*window_offset 2*window_offset full_window_size(3)-window_offset_size ...
            full_window_size(4)-window_offset_size], 'Name', ...
            'Метод внешних штрафных функций (квадратичная ф.)', ...
            'NumberTitle', 'off'); %c
    case 2
        figure('Position', [6*window_offset 6*window_offset full_window_size(3)-window_offset_size ...
            full_window_size(4)-window_offset_size], 'Name', ...
            strcat('Метод внешних штрафных функций (ф. Розенброка, alpha = ', num2str(alpha), ')'), ...
            'NumberTitle', 'off'); %c
    case 3
        figure('Position', [2*window_offset 2*window_offset full_window_size(3)-window_offset_size ...
            full_window_size(4)-window_offset_size], 'Name', ...
            'Метод внешних штрафных функций (ф. Химмельблау)', ...
            'NumberTitle', 'off'); %c
    otherwise
        disp('ERROR!')
end
...
if choose_function == 2
    ...
    figure('Position', [2*window_offset 2*window_offset full_window_size(3)-window_offset_size ...
        full_window_size(4)-window_offset_size], 'Name', ...
        strcat('Метод внешних штрафных функций (ф. Розенброка, alpha = ', ...
...
function  [X, norm_w, x_k, y_k,count_f_xy,count_new_dots, ...
    ...
    switch case_num
        case 1
            % 1. метод внутренних штрафных (барьерных) функций
            % MethodInternalPenaltyFunc
            inf_ = 1000000000;
            gamma = 2.;
            r_k = 1; %c
        
            g_xy=@(x,y) 3.*(x.^3./4)+5.*(x.^1./4)-2-y;
            g = g_xy(x, y);
        
            ConditionsIneq = [ConditionsIneq, g];
            ConditionsEq = [];
        
            NumCond = length(ConditionsIneq) + length(ConditionsEq); %(*количество условий*)
        
            h_xy=@(x,y) 0;
            h = h_xy(x, y);
            for i = 1:NumCond
                h = h + 1/ConditionsIneq(i);
            end
        
            F = double(subs(f, Arg, X));
            ...
            while 1
                ...
                [X, norm_w0, x_k0, y_k0,count_f_xy0,count_new_dots0, ...
                    count_grad_f_xy0, count_hess0] = method_Newton(f_curr, X, eps, kappa0, ...
                    25, nu, omega, 1); %с
                count_f_xy = count_f_xy + count_f_xy0;
                count_grad_f_xy= count_grad_f_xy + count_grad_f_xy0;
                count_hess=count_hess+count_hess0;
...
function draw_surf_coutour_and_steps(f, X0, x_k, y_k, ~, e, time_needed)
    ...
    border_5 = 10;
    [X, Y] = meshgrid(-2*x_max-border_5:0.03:2*x_max+border_5, ...
        -2*x_max-border_5:0.03:2*x_max+border_5);
...
% метод Ньютона %c
function  [X, norm_w, x_k, y_k,count_f_xy,count_new_dots, ...
        count_grad_f_xy, count_hess] = method_Newton(f0, X0, eps, kappa0, ...
    ...
    syms x y kappa
    Arg = [x; y];
    antigrad_f0 = -gradient(f0);  % антиградиент
   
    % Hessian matrix
    H = [diff(f0, x, {2}),     diff(diff(f0, x), y);
         diff(diff(f0, y), x),     diff(f0, y, {2})];
    count_hess = count_hess + 1;
    
    W = double(subs(antigrad_f0, Arg, X));
    count_grad_f_xy=count_grad_f_xy+1;
    f_k = double(subs(f0, Arg, X));
    count_f_xy=count_f_xy+1;
    ...
    while norm(W) > eps  
        ...
        switch case_num
            ...
            case 2
                Xcurr = X + kappa_k.*p;
                while f_k - double(subs(f0, Arg, Xcurr)) <= ...
